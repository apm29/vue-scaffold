const TOKEN_KEY = "ZAXQ_TOKEN_KEY";
const USER_INFO_KEY = "ZAXQ_USER_INFO_KEY";
const DISTRICT_KEY = "ZAXQ_DISTRICT_KEY";
...
export const setDistrictList = (districtList) => {
  setLocalStorage(DISTRICT_KEY, JSON.stringify(districtList));
};

export const getDistrictList = () => {
  const districtListString = getLocalStorage(DISTRICT_KEY);
  if (districtListString) {
    try {
      return JSON.parse(districtListString);
    } catch (e) {
      console.log(e);
      return [];
    }
